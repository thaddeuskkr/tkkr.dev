...
const navigationItemClassName =
    'rounded-lg px-2 text-muted-foreground hover:bg-background/30 hover:text-foreground motion-safe:hover:-translate-y-px min-[480px]:px-2.5 dark:hover:bg-white/5';
...
export function Navigation() {
    ...
    return (
        ...
        >
            <div className="pointer-events-auto mx-auto mt-4 grid h-14 w-full max-w-4xl grid-cols-[1fr_auto_1fr] items-center rounded-xl border border-border/70 bg-background/75 px-2 shadow-[0_12px_40px_-26px_var(--foreground)] backdrop-blur-lg backdrop-saturate-150 transition-[border-color,background-color,box-shadow] duration-500 hover:border-primary/20 supports-backdrop-filter:bg-background/65 sm:mt-5 sm:px-2.5">
                <div className="flex gap-0.5">
                    {navigationLinks.map(({ to, label, icon: Icon }) => (
                        ...
                        >
                            <Icon className="transition-transform duration-300 group-hover/button:scale-110 min-[480px]:mr-0.5" />
                    ...
                    <Button aria-label="Projects" variant="ghost" size="lg" className={navigationItemClassName}>
                        <Briefcase className="transition-transform duration-300 group-hover/button:scale-110 min-[480px]:mr-0.5" />
                        <span className="hidden min-[480px]:inline">Projects</span>
                    </Button>
                    */}
                </div>
                ...
                <RouterLink
                    to="/"
                    aria-label="tkkr.dev home"
                    className={cn(
                        buttonVariants({ variant: 'ghost', size: 'icon-lg' }),
                        'relative justify-self-center rounded-lg'
                    )}
                >
                    <span
                        aria-hidden="true"
                        className="block size-7 bg-primary transition-[transform,background-color] duration-300 [-webkit-mask:url('/icon-dark.svg')_center/contain_no-repeat] [mask:url('/icon-dark.svg')_center/contain_no-repeat] group-hover/button:-translate-y-px group-hover/button:scale-105 motion-reduce:transition-none"
                    />
                </RouterLink>
